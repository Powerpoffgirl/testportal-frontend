...
export default function DescriptionSummary() {
  ...
  //           `${baseUrl}/api/v1/doctor/send_report/${doctorId}`,
  //           {
  //             method: "POST",
  //             headers: {
  //               "x-auth-token": token,
  ...
  const handleSms = async (appointmentId) => {
    try {
      const token = localStorage.getItem("token");
      if (!token) {
      ...
      const response = await fetch(
        `${baseUrl}/api/v1/doctor/send_report/${appointmentId}`,
        {
          method: "GET",
          headers: {
      ...
      >
        <div className="MainContainer" style={{ width: "100%" }}>
          ...
          >
                    ...
                    <div class="flex flex-col ">
                      ...
                      <div class="">
                        <p class="flex flex-wrap space-x-1 w-64">
                          <p>{appointment?.doctorId?.address?.houseNo}</p>
                          <p>{appointment?.doctorId?.address?.floor}</p>
                          <p>{appointment?.doctorId?.address?.block}</p>
                          <p>{appointment?.doctorId?.address?.area}</p>
                          <p>{appointment?.doctorId?.address?.district}</p>
                          <p>{appointment?.doctorId?.address?.state}</p>
                          <p>{appointment?.doctorId?.address?.pinCode}</p>
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <table className=" divide-y divide-gray-200 border border-[#89CFF0]">
                    <thead className="bg-[#89CFF0]">
                      <tr>
                        ...
                        <th className="px-2 py-3 text-left text-base font-medium text-white uppercase tracking-wider">
                          Issues
                        </th>
                        <th className="px-2 py-3 text-left text-base font-medium text-white uppercase tracking-wider">
                          Disease
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      ...
                      <td className="w-[300px] px-2 py-4 whitespace-normal text-sm text-black break-words">
                        {appointment?.issues.join(", ")}
                      </td>
                      <td className="w-[300px] px-2 py-4 whitespace-normal text-sm text-black break-words">
                        {appointment?.diseases.join(", ")}
                      </td>
                      <td className="w-[300px] px-2 py-4 whitespace-normal text-sm text-black break-words">
                        {appointment?.medicineName.join(", ")}
                      </td>
                      <td className="w-[300px] px-2 py-4 whitespace-normal text-sm text-black break-words">
                        {appointment?.labTests?.join(", ")}
                      </td>
                    </tbody>
                  </table>
                </div>
            ...
            >
              <Tooltip title="Go back to patient description">
                <button
                  onClick={() =>
                    navigate(`/patientdescription/${appointment?._id}`, {
              ...
              <Tooltip title="Send report via SMS">
                <button onClick={() => handleSms(appointment?._id)}>
                  <label
                    htmlFor="files"
                    id="sendToSMSButton"
