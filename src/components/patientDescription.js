...
export default function PatientDescription() {
  ...
  const handleRegister = async (e) => {
    ...
    if (
      ...
      Array.isArray(patientDetails?.medicineName) &&
      patientDetails?.medicineName?.length === 0
    ) {
      toast.error("Please select medicineName!");
    } else if (
      Array.isArray(patientDetails?.labTests) &&
      patientDetails?.labTests?.length === 0
    ) {
      toast.error("Please select labTests!");
    } else {
