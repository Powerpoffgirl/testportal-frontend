      ...
      try {
        ...
        const test = rows?.filter(
          (row) =>
            row?.testName
              ?.toLowerCase()
              .includes(lastItem?.testName.toLowerCase()) &&
            row?.testCode
              ?.toLowerCase()
              .includes(lastItem?.testCode.toLowerCase())
        );
        console.log("==================TEST===========================", test);
        // if (test.length > 0)
        // {
        ...
        // {
        const response = await fetch(
          `${baseUrl}/api/v1/lab/create_testBooking`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
  ...
  useEffect(() => {
    const fetchTestDetails = async () => {
      try {
        ...
        const testList = responseData.data?.filter(
  ...
  return (
    <>
      <div
        ...
        class={`  md:max-w-xl lg:max-w-2xl xl:max-w-5xl 2xl:max-w-full max-w-[400px] `}
      >
