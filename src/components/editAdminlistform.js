...
export default function EditAdminlistform()
{
    ...
    useEffect(() =>
    {
        const fetchPatientDetails = async () =>
        {
            try
            {
                ...
                const response = await fetch(`${baseUrl}/api/v1//superAdmin/admin_byId/${patientId}`, {
                    method: 'GET',
                    headers: {
    ...
    const handleChange = (e) =>
    {
        ...
        } else if (["issues"].includes(name))
        {
            ...
            setAdminDetails(prevAdminDetails => ({
                ...prevAdminDetails,
                [name]: Array.isArray(value) ? value : [...prevAdminDetails[name], value]
            }));
    ...
    const handleRegister = async (e) =>
    {
        ...
        const newAdminDetails = {
            ...
            permissions: {
                view: adminDetails.permissions.view,
                create: adminDetails.permissions.create,
                remove: adminDetails.permissions.remove,
                edit: adminDetails.permissions.edit,
            },
            address: {
                houseNo: adminDetails?.address?.houseNo,
                floor: adminDetails?.address?.floor,
                block: adminDetails?.address?.block,
                area: adminDetails?.address?.area,
                pinCode: adminDetails?.address?.pinCode,
                district: adminDetails?.address?.district,
                state: adminDetails?.address?.state
            },
            adminPic: adminImage
        }

        console.log("admin details######", newAdminDetails)

        // Check if the token exists
        const token = localStorage.getItem("token");
        const patientId = localStorage.getItem("patientId");

        const isEmpty = Object.values(newAdminDetails).some(value => value === '');
        ...
        const response = await fetch(
            `${baseUrl}/api/v1/superAdmin/update_admin/${patientId}`,
            {
                method: "put",
                headers: {
                    "Content-Type": "application/json",
